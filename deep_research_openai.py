"""
Deep research agent using OpenAI Agents SDK and Firecrawl.

This module implements a comprehensive web research system that performs deep
research on any topic using Firecrawl's deep research endpoint and OpenAI's
Agents SDK for intelligent analysis and elaboration.
"""
...
st.markdown(
    "This agent performs deep research on any topic using OpenAI Agents SDK and Firecrawl"
)
...
async def deep_research(
    ...
    Args:
        ...
        max_depth: Maximum depth for research
        time_limit: Time limit in seconds
        max_urls: Maximum number of URLs to research

    Returns:
        dict: Research results including analysis and sources
    """
    try:
        ...
        def on_activity(activity: Dict[str, Any]) -> None:
            """Handle activity updates from Firecrawl."""
...
if st.button(
    "Start Research",
    disabled=not (st.session_state.openai_api_key and
                  st.session_state.firecrawl_api_key and
                  research_topic)
):
    if not st.session_state.openai_api_key or not st.session_state.firecrawl_api_key:
        ...
        try:
            ...
            logger.exception("Error during research process")

# Footer
st.markdown("---")
st.markdown(
    "Powered by [OpenAI Agents SDK](https://platform.openai.com/) and "
    "[Firecrawl](https://www.firecrawl.dev/) by [Rishi Chhabra](https://github.com/rchhabra13)"
)
